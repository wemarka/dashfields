...
export function CampaignDetailDrawer({ campaign, open, onClose }: Props) {
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-4xl flex flex-col overflow-hidden border-l border-border bg-background p-0"
      >
        {/* ── Header ──────────────────────────────────────────────────── */}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <nav className="w-44 shrink-0 flex flex-col border-r border-border bg-muted/20 overflow-y-auto">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="text-[10px] text-muted-foreground/60 leading-tight truncate">{tab.desc}</span>
                    </div>
                    {/* Green dot: data prefetched and ready */}
                    {isLoaded && !isActive && (
                      <span className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" />
                    )}
                  </button>
                );
              })}
