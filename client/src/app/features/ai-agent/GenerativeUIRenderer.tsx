  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">{block.title}</span>
      </div>
      ...
      {block.subtitle && <span className="text-xs text-gray-400 mt-1 block">{block.subtitle}</span>}
    </div>
  );
}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50/80">
              {block.columns.map((col, i) => (
                <th key={i} className="px-4 py-2.5 text-start text-xs font-medium text-gray-500 uppercase tracking-wide">
                  {col}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {block.rows.map((row, ri) => (
              <tr key={ri} className="border-t border-gray-50 hover:bg-gray-50/40 transition-colors">
                {row.map((cell, ci) => (
...
function SimpleBarChart({ block }: { block: BarChartBlock }) {
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      {block.title && <span className="text-sm font-semibold text-gray-700 mb-3 block">{block.title}</span>}
      <div className="space-y-2.5">
        {block.labels.map((label, i) => (
          <div key={i} className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium text-gray-700 w-12 text-end">{block.values[i]}</span>
          </div>
        ))}
      </div>
    </div>
...
function InfoCard({ block }: { block: InfoCardBlock }) {
  return (
    <div className="rounded-xl border border-gray-100 bg-white overflow-hidden shadow-sm">
      ...
      <div className="p-4">
        <h4 className="text-sm font-semibold text-gray-800 mb-1">{block.title}</h4>
      ...
      <div className="grid grid-cols-2 gap-2">
        {block.images.map((img, i) => (
          <div key={i} className="rounded-lg overflow-hidden bg-gray-50">
            ...
            {img.caption && <span className="text-[10px] text-gray-500 px-2 py-1 block">{img.caption}</span>}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-sm font-semibold text-gray-800">{block.name}</h4>
        ...
        {block.budget && (
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-xs font-medium text-gray-700">{block.budget}</span>
            </div>
          </div>
        )}
        {block.reach && (
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-xs font-medium text-gray-700">{block.reach}</span>
            </div>
          </div>
        )}
        {block.clicks && (
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-xs font-medium text-gray-700">{block.clicks}</span>
            </div>
          </div>
        )}
        {block.conversions && (
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-[10px] text-gray-400 block">Conversions</span>
              <span className="text-xs font-medium text-gray-700">{block.conversions}</span>
            </div>
          </div>
        )}
      </div>
